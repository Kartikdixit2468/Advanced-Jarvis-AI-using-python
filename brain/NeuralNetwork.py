...
try:
    ...
    " the torch version given in requirements.txt and then try agin.")
    flag =  False
...
if flag:
    ...
    class NeuralNet(nn.Module):
        """ This class gives our jarvis 
        ability to think to Jarvis.  """
        ...
        def forward(self, x):
            """ This function connect x: Query to 
            each layer of our Neural Network. """
            output = self.l1(x)
            output = self.relucate(output) 
            output = self.l2(output)    # Connecting each layer by framing neurons.
            output = self.relucate(output)
